        ...
        match self.0[0] {
            CodeRef::Extern(ext) => {
                if let Some(ext) = ext.access(prog) {
                    match ext {
                        ExternEntry::Value {value,..} => Ok(value()),
                        _ => bail!("Not a value")
                    }
                } else {
                    bail!("Invalid extern")
...
impl<F> Debug for WrappedFn<F> where F: FnOnce(&Program, Context, u8) -> Result<(CodeRef, Context), Error> {
    fn fmt(&self, fmt: &mut Formatter) -> core::fmt::Result {
        write!(fmt, "{}", self.0)
    }
}
impl<F> Value for WrappedFn<F> where F: FnOnce(&Program, Context, u8) -> Result<(CodeRef, Context), Error> + 'static {
    fn eval(
        self: Box<Self>,
        p: &Program,
        ctx: Context,
...
pub fn native_closure(name: impl Into<String>, f: impl FnOnce(&Program, Context, u8) -> Result<(CodeRef, Context), Error> + 'static) -> Box<dyn Value> {
